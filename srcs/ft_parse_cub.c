...
{
	if (!(*scene = ft_calloc(1, sizeof(**scene))))
		return (0);
	else if (!(*status = ft_calloc(1, sizeof(**status))))
	{
		free(*scene);
		return (0);
	}
...
t_scene	*ft_parse_cub(t_game_data *data, const char *filename)
{
	...
	if (ft_check_file_extention(filename, ".cub") == 0)
		return (NULL);
	if (!(ft_init_scene_status(&(data->scene), &status)) ||
		(fd = open(filename, O_RDONLY)) == -1)
		return (ft_free_all(&(data->scene), &status, NULL, NULL));
	while (get_next_line(fd, &line) == 1)
	{
		splited_data = ft_split(line, ' ');
		if (!ft_check_scene_element(&(data->scene), splited_data, &status))
			return (ft_free_all(&(data->scene), &status, &line, splited_data));
		ft_free_all(NULL, NULL, &line, splited_data);
	}
	ft_check_n_parse_map(data, &status);
	close(fd);
	if (ft_check_status(&status) == -1)
		return (ft_free_all(&(data->scene), &status, &line, NULL));
